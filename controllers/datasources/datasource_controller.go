...
func (r *PersesDatasourceReconciler) syncPersesDatasource(ctx context.Context, perses persesv1alpha1.Perses, datasource *persesv1alpha1.PersesDatasource) (*ctrl.Result, error) {
	...
	datasourcedWithName := &datasource.Spec.Datasource
	datasourcedWithName.Metadata.Name = datasource.Name

	if err != nil {
		if errors.Is(err, perseshttp.RequestNotFoundError) {
			_, err = persesClient.Datasource(datasource.Namespace).Create(datasourcedWithName)
		...
		_, err = persesClient.Datasource(datasource.Namespace).Update(datasourcedWithName)
